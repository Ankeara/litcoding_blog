                ...
                <div class="col-12 col-md-12 col-lg-12">
                    <div class="card">
                        ...
                        <div class="card-header d-flex align-items-center justify-content-between">
                            ...
                            <a href="{{ route('template.template.create') }}" class="btn btn-primary btn-sm">Create</a>
                        </div>
                        <div class="card-body">
                                    ...
                                    @if ($template->isNotEmpty())
                                        @foreach ($template as $item)
                                            <tr>
                                                ...
                                                <td><img src="{{ url('/default/template/' . $item->image_template) }}"
                                                    ...
                                                    <div class="dropdown d-inline">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            <a class="dropdown-item has-icon"
                                                                href="{{ route('template.template.edit', $item->id) }}"><i
                                                                    class="fas fa-pencil-alt"></i> Update</a>
                                                            <a class="dropdown-item has-icon" href="#"
                                                                onclick="deletetemplate('{{ $item->id }}')"><i
                        ...
                        <div class="card-footer text-right">
                            {{ $template->links('pagination::bootstrap-5') }}
                        </div>
                    </div>
                </div>
            </div>
...
@section('customJS')
    <script>
        function deletetemplate(jsid) {
            // Set the job id in the modal
            $('#deletetemplateModal').data('jsid', jsid);
            // Show the modal
            $('#deletetemplateModal').modal('show');
        }

        // Handle deletion confirmation in the modal
        $('#confirmDeletetemplateBtn').click(function() {
            var jsid = $('#deletetemplateModal').data('jsid');
            // Send an AJAX request to delete the job
            $.ajax({
                url: '/admin/template/delete/' + jsid,
                type: 'DELETE',
                headers: {
